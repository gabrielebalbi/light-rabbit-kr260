#!/usr/bin/env bash
# deploy_v12.sh — DA ESEGUIRE SULLA SCHEDA come root:  sudo bash ~/deploy_v12.sh
# Installa e carica kr260_wr_sdm v12: wrc ricompilato da master con STACK 4096
...
set -u
if [ "$(id -u)" -ne 0 ]; then echo "!! Va lanciato come root:  sudo bash ~/deploy_v12.sh"; exit 1; fi
APP=kr260_wr_sdm
SRC=/home/ubuntu/kr260_wr_sdm_app
DST=/lib/firmware/xilinx/$APP
STAMP=$(date +%Y%m%d_%H%M%S)
LOG=/home/ubuntu/out_v12.log
exec > >(tee "$LOG") 2>&1
W(){ python3 /home/ubuntu/wrc_cmd.py "$1"; sleep 1; }

echo "=================================================================="
echo " DEPLOY v12 (wrc master stack4096, an=0, +mdio)  —  $(date '+%H:%M:%S')"
