#!/usr/bin/env bash
# test_zen_v15.sh — DA ESEGUIRE SULLA SCHEDA come root:  sudo bash ~/test_zen_v15.sh
# Test link WR col partner WR-ZEN, fw v15 = v14 (fix DAC 16->20bit + bitslide) + LED1/LED2.
...
#
# LED (v15) — lettura a colpo d'occhio, senza terminale:
#   LED2 (F7, cage SFP) respira            -> bitstream vivo (subito dopo il load)
#   UF2  (E8)           fisso              -> link WR su       (respira = sta cercando)
#   UF1  (F8)           lampeggia 1/s      -> PPS attivo
#   LED1 (G8, cage SFP) FISSO              -> tm_time_valid = SINCRONIZZAZIONE VALIDA
#                                             (il segnale visivo dell'obiettivo MPL1)
#
...
set -u
if [ "$(id -u)" -ne 0 ]; then echo "!! Va lanciato come root:  sudo bash ~/test_zen_v15.sh"; exit 1; fi
APP=kr260_wr_sdm_app_v15
LOG=/home/ubuntu/out_zen_v15.log
exec > >(tee "$LOG") 2>&1
W(){ python3 /home/ubuntu/wrc_cmd.py "$1"; sleep 1; }

echo "=================================================================="
echo " TEST WR-ZEN (fw v15: DAC 20bit + bitslide + LED)  --  $(date '+%H:%M:%S')"
echo " SETUP: >>> FIBRA VERSO WR-ZEN (SFP BiDi) <<<   (NON loopback!)"
echo "=================================================================="

echo "== reload app v15 (reset pulito del GT/PCS) =="
...
echo ""
echo ">>> GUARDA LA SCHEDA: LED2 (cage SFP) deve RESPIRARE ora = fabric vivo."
echo ""

echo "== LASER on (il redeploy azzera il GPO) =="
python3 /home/ubuntu/tx_enable.py on 2>&1 | tail -2

echo "== attesa 6 s boot wrc =="; sleep 6
W "ptp stop"
...
echo "   ATTESO: lnk:1 (UF2 fisso), poi pll stat con MFL1 e finalmente MPL1"
echo "   (m_phase_lock_ms>0) -> a quel punto LED1 sulla cage si accende FISSO."
...
echo " Sulla scheda: LED1 (cage) FISSO, UF2 fisso, UF1 lampeggia 1/s."
echo " Se MFL1 ma MPL0 persistente -> attivare il BLOCCO FALLBACK (pll gain shift 16+)."
echo "== FINE test ZEN v15 =="
